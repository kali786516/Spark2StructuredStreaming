package com.dataframe.part21.RealTimeFraudDetection.fraudDetection.spark.jobs.realTimeFraudDetection

import com.dataframe.part21.RealTimeFraudDetection.fraudDetection.cassandra.dao.{CreditcardTransactionRepository, KafkaOffsetRepository}
...
/**
  ...
  */
object DstreamFraudDetection extends SparkJob("Feaud Detection using Dstream"){
  ...
  val connector            = CassandraConnector(sparkSession.sparkContext.getConf)
    ...
    transactionStream.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        println("-" * 100)
        println("featureTransactionDF")
        featureTransactionDF.show(10,false)
