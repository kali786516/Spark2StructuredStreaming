...
object ComcastSparkCassandra {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val op=spark.sql("" +
      "SELECT 1 as column_order,min(column1) as min_value," +
      "percentile(cast(column1 as BIGINT), 0.25) as quartile1,percentile(cast(column1 as BIGINT), 0.5) as median,percentile(cast(column1 as BIGINT), 0.75) as quartile3,regexp_replace(max(column1), \"NaN\", 0) as max_value,current_timestamp as created_ts FROM table " +
      "union all " +
